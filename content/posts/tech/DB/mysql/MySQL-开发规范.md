+++
date = '2024-06-28T21:49:45+08:00'
draft = false
title = 'MySQL 开发规范'
slug = "k2yhizsqk4"
description = "MySQL 开发规范"
categories = ["📒数据库"]
tags = ["mysql"]
summary = "MySQL 开发规范"
series = ["MySql数据库学习系列"]
...
- 建议使用 INT UNSIGNED 类型存储 IPV4 地址。用 UNSINGED INT 存储 IP 地址占用 4 字节，`CHAR(15)` 则占用 15 字节。另外，计算机处理整数类型比字符串类型快。
> Tips: MySQL 提供函数 `inet_ntoa` 和 `inet_aton` 来对 ip 地址在 int 与 char 之间相互转化。IPv6 地址目前没有转化函数，需要使用 DECIMAL 或两个 BIGINT 来存储。
