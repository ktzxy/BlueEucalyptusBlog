+++
date = '2026-02-18T20:41:31+08:00'
draft = false
title = '单元测试'
slug = "6xkk74e3qy"
description = "单元测试"
...
上面的命令会将覆盖率相关的信息输出到当前文件夹下面的`c.out`文件中，然后我们执行`go tool cover -html=c.out`，使用`cover`工具来处理生成的记录信息，该命令会打开本地的浏览器窗口生成一个HTML报告。![Go test cover](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202507291453160_7c7b02.webp)上图中每个用绿色标记的语句块表示被覆盖了，而红色的表示没有被覆盖。
...
3. 示例函数提供了可以直接运行的示例代码，可以直接在`golang.org`的`godoc`文档服务器上使用`Go Playground`运行示例代码。下图为`strings.ToUpper`函数在Playground的示例函数效果。![Go Playground](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202507291453161_55fe83.webp)
