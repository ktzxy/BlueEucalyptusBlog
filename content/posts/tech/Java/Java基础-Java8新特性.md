+++
date = '2024-02-21T01:56:18+08:00'
draft = false
title = 'Java基础 Java8新特性'
slug = "9x1w9m6qae"
description = "Java基础 Java8新特性"
categories = ["📒开发"]
tags = ["Java"]
summary = "Java基础 Java8新特性"
comments = true  
+++
## 1. Java 8 新特性概述
...
![Stream 流程图](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035325_46de06.webp)
...
![流方法分类](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035326_237cd8.webp)
...
![](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035327_904086.webp)
...
![](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035328_106c11.webp)
...
#### 6.7.10. Collectors.toList() 源码解析（了解）
...
以上学习使用的 Stream 流是串行的，就是在一个线程上执行。
...
![](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035329_c7b203.webp)
...
#### 6.8.5. parallelStream 底层实现原理

> 注：parallelStream底层实现是使用Fork/Join框架，此框架的详细学习笔记详见《并发编程》中的相关笔记

##### 6.8.5.1. Fork/Join 框架介绍
...
![](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035330_f78b3c.webp)

##### 6.8.5.2. Fork/Join 原理-分治法
...
![](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035331_42596d.webp)

##### 6.8.5.3. Fork/Join 原理-工作窃取算法

Fork/Join最核心的地方就是如何利用多核的处理器。工作窃取（work-stealing）算法就是整个Fork/Join框架的核心理念。Fork/Join工作窃取（work-stealing）算法是指某个线程从其他队列里窃取任务来执行

![](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035332_ad45e9.webp)
...
2. parallelStream 适用的场景 是CPU 密集型的，只是做到别浪费 CPU，假如本身电脑 CPU 的负载很大，那还到处用并行流，那并不能起到作用
3. 磁盘 I/O、网络 I/O 都属于密集型 I/O 操作，这部分操作是较少消耗 CPU 资源，一般并行流中不适用于 I/O 密集型的操作，就比如使用并流行进行大批量的消息推送，涉及到了大量 I/O，使用并行流反而慢了很多
...
![](https://fastly.jsdelivr.net/gh/ktzxy/blog-img@main/2026/202410251035333_d2d24b.webp)
...
#### 8.4.2. orElse()、orElseGet() 与 orElseThrow()
...
自从 Java 5 中引入注解以来，在各个框架和项目中被广泛使用。不过注解有一个很大的限制是：在同一个地方不能多次使用同一个注解。JDK 8 引入了重复注解的概念，允许在同一个地方多次使用同一个注解

在 JDK 8 中使用`@Repeatable`注解定义重复注解。
